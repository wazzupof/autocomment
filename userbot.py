...
spam_posts = [TextPost("Спасибо за пост!")]
# App - stores Pyrogram instance
app = Client("data/my_account", api_id=22725919,
             api_hash="89cf91b2b510b837e15210ad6293c406")
...
async def answer(_, message):
    ...
    logger.info(
        f"|{message.chat.title}|:Caught message. (Text: {message_log})")

    last_media_group = message.media_group_id
    await sleep_if_required(10)
    linked = await get_linked(message)
    if linked:
        message_to_answer = await get_forwarded_in_linked(message.id, linked)
    else:
        return
    if message_to_answer is None:
        logger.error(
            f"|{message.chat.title}|:Banned or couldn't get comments.")
        return
...
# TODO: Check for forwarded_from_id
async def get_forwarded_in_linked(message_id, linked):
    try:
        async for message in app.search_messages(linked.id, limit=1, filter=enums.MessagesFilter.PINNED):
            return message
